...
/*jshint unused: vars*/
var encontrarEnArray = function(list,criteria,errorMsg){
	var filtered = list.filter(
			function(element,index,array){return criteria(element);});
...
var Actividad = Ember.Object.extend({
  init: function() {
    ...
    this.set('nombre', descripcion.nombre);
    this.set('enunciado', descripcion.enunciado);
    this.set('escena', descripcion.escena);
    this.set('puedeComentar', descripcion.puedeComentar);
    this.set('puedeDesactivar', descripcion.puedeDesactivar);
    this.set('puedeDuplicar', descripcion.puedeDuplicar);
    this.setColours();
  ...
  usa_procedimientos: function() {
    return this.get('descripcion').subtareas.indexOf(Procedimiento) > -1;

  },

  usa_funciones: function() {
    return this.get('descripcion').subtareas.indexOf(Funcion) > -1;
  },
...
/* ============================================== */
var listaDescripciones = [descripcionActividadAlien];

export default Ember.Object.extend({
	fromId: function(idActividad){
		return Actividad.create({ descripcion: this.findDescripcion(idActividad) });
	},

	findDescripcion: function(idActividad){
		return encontrarEnArray(
				listaDescripciones,
				function(act){return act.id === idActividad;},
